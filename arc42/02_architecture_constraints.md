...
| **TC-3: GPIO Limitations** | Only 7 usable GPIO pins (GPIO 0 reserved for boot button, GPIO 15 not usable for PWM) | Reduces output count from ESP32's 16 to ESP8266's 7. |
| **TC-4: Single Core** | ESP8266 is single-threaded (vs. ESP32 dual-core) | Cannot offload WebSocket handling or web server to separate core. Requires yield() calls in main loop. |
| **TC-5: PWM API** | ESP8266 uses `analogWrite()` with 10-bit range (0-1023), not ESP32's `ledcWrite()` | Requires different PWM setup code. Uses 8-bit range (0-255) to save memory. |
...
| **SC-2: EEPROM API** | ESP8266 uses `EEPROM.h` (vs. ESP32's Preferences library) | More manual memory management. Single 512-byte structure. |
...
|------------|----------|------------------------|-----------|
| **Microcontroller** | ESP8266 (ESP-12E) | ESP32, STM32 | Low cost (~$2), WiFi built-in, large community. |
